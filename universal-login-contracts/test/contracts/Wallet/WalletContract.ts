...
import {calculateMessageHash, calculateMessageSignature, DEFAULT_GAS_PRICE, TEST_ACCOUNT_ADDRESS, UnsignedMessage, signString, createKeyPair, createSignedMessage, SignedMessage} from '@universal-login/commons';
...
describe('WalletContract', async () => {
  ...
  describe('Upgrade', () => {
    ...
    beforeEach(async () => {
      ...
      signedUpdateMessage = createSignedMessage(await setupUpdateMessage(proxyAsWalletContract, newWallet.address), privateKey);
    ...
    it('updates master', async () => {
      const gasLimit = estimateGasLimit(signedUpdateMessage.gasLimitExecution, signedUpdateMessage.gasData);
  ...
  describe('gasLimitExecution', () => {
    it('reject execution that has too small gasLimit', async () => {
      ...
      await expect(wallet.sendTransaction({to: walletContractProxy.address, data, gasLimit: tooLowGasLimit})).to.be.eventually.rejectedWith('Gas limit too low');
      expect(await provider.getBalance(TEST_ACCOUNT_ADDRESS)).to.eq(0);
    });
